...
`GATEWAY_WEBHOOK_SECRET_DIRECTORY` names a dedicated, externally provisioned
directory that is mounted read-only on every node. Each regular file must be named
`kong-webhook-<integration-id>`, contain a single secret of at least 32 characters,
and is exported only in-process using the corresponding uppercase env Vault key.
Signed-webhook contracts must use
`{vault://env/kong-webhook-<integration-id>}`; cross-integration references are
rejected so every integration has an isolated signing key. Rotate a key by updating
only its file through the approved secret-management process and restarting nodes.
