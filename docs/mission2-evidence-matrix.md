...
| Secret scan | `SECRET_HITS=0` over the authority roots (sentinels only in tests); gitleaks 8.30.1 (`security.yml`) on PR #107 reported 4 findings, all the profile identifier `INTERNAL_STANDBY_V1` on the four standby routes (entropy 3.68, `generic-api-key` heuristic, commit `8d4ac7b`); `.gitleaks.toml` extends the default rules and exempts exactly the one JSON key that names an authentication profile, whose values the foundation validator restricts to catalogue names, and `.gitleaksignore` pins the single documentation line of commit `45b3286` that quoted the pair — verified with the pinned binary that a private-key block beside the exempted line is still reported | GREEN |
