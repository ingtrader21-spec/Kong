...
| `OIDC_STRIP_AND_CONTRACT_METADATA` | **yes** — the generated post-function clears every client-asserted identity header (`X-User-ID`, `X-Authenticated-*`, `X-Codestra-Tenant/-Scopes/-Gateway-Secret`, `X-Internal-Service`, `X-Admin`, …) before minting; `X-Tenant-ID` stays a selector | `X-Consumer-*` (openid-connect), `X-Codestra-Contract-Operation`, `X-Codestra-Expected-Azp`, `X-Codestra-Required-Scope` | the 80 canonical `middleware-*` routes (production and staging) |
| `KONG_CONSUMER_HEADERS_ONLY` | **no** — client `X-Authenticated-*` pass through; Kong overwrites `X-Consumer-*` after the signature check | `X-Consumer-ID/-Username`, `X-Credential-Identifier` | transitional/retiring jwt routes (callbacks, campaign, n8n), intake, prepared provider control (accepted finding `IDENTITY_HEADERS_NOT_STRIPPED`, mitigation: Middleware ignores them and re-validates the token; superseded by the v2 routes rather than refactored) |
